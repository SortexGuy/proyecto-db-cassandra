...
func HybridRecommendation(g *simple.UndirectedGraph, initialUserID int64, steps int, lambda float64, num_recommendations int) []int64 {
	...
	sortedScores := sortMapDescending(hybridScores)
	recommendations := make([]int64, 0, num_recommendations)

	for i := 0; i < len(sortedScores) && i < num_recommendations; i++ {
		recommendations = append(recommendations, sortedScores[i].ID)
	}
