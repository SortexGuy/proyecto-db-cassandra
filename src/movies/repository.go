...
func findMovieByIDRepository(id int64) (MovieDTO, error) {
	movie := MovieDTO{}
	...
	for _, m := range movies {
		if m.ID == id { // Asegúrate de que el tipo coincida
...
func getAllMoviesByUserRepository(userID int64) ([]MovieByUser, error) {
	session := config.SESSION
