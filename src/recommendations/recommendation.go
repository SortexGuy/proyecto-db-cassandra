...
func CreateGraph(users, movies []int64, relations []movies.MovieByUser) *simple.UndirectedGraph {
...
func sortMapDescending(hybridScores map[int64]float64, numRecommendations int) []int64 {
	...
	result := make([]int64, 0, numRecommendations)
	for i, item := range items {
		if i < numRecommendations {
			result = append(result, item.ID)
		} else {
			break // Ya tenemos las numRecommendations mejores
		}
	}
...
func fillUserMovies(numUsers, movies, maxMoviesPerUser int) []UserMovie {
	userMovies := make([]UserMovie, 0)
	for userID := int64(1); userID <= int64(numUsers); userID++ {
		// Número aleatorio de películas para este usuario (hasta maxMoviesPerUser)
		numMovies := rand.Intn(maxMoviesPerUser) // +1 para asegurar al menos una película

		// Generar películas únicas para este usuario
		movieIDs := make(map[int64]bool)
		for i := 0; i < numMovies; i++ {
			var movieID int64
			for {
				movieID = rand.Int63n((int64(movies - 1))) + 1 // Genera IDs de película entre 0 y 999 (ajusta según tus necesidades)
				if !movieIDs[movieID] {
					movieIDs[movieID] = true
					break
				}
			}
			userMovies = append(userMovies, UserMovie{UserID: userID, MovieID: movieID})
		}
	}
	return userMovies
}
